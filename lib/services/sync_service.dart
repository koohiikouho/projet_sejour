    ...
    try {
      ...
      for (var tripDoc in tripsSnapshot.docs) {
        try {
          ...
          final daysSnapshot = await tripDoc.reference.collection('itineraryDays').get();
              ...
              for (var activityDoc in activitiesSnapshot.docs) {
                try {
                  ...
                  final activity = Activity(
                    ...
                    mobilityRating: activityData['mobilityRating'] ?? 0,
                    location: activityData['location'] ?? '',
                    scheduledArrival: _parseDate(activityData['scheduledArrival']),
                    scheduledDeparture: _parseDate(activityData['scheduledDeparture']),
                    whatToBring: List<String>.from(activityData['whatToBring'] ?? []),
